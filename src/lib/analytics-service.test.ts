...
describe("adherenceRate", () => {
  ...
  it("excludes future pending slots from denominator when date is today (D-16)", async () => {
    // Set system time to 2024-06-01 07:00 UTC.
    // In the UTC+2 test environment this is 09:00 local, so 14:00 and 18:00
    // local slots are clearly future (local now = 540 min; 14:00 = 840 min).
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2024-06-01T07:00:00Z"));

    try {
      const todayStr = "2024-06-01";
      ...
      const map = new Map<string, DoseSlot[]>();
      // 08:00 local — past (540 min local now > 480) → counted, taken
      // 14:00 local — future (840 > 540), pending → excluded from denominator
      // 18:00 local — future (1080 > 540), pending → excluded from denominator
      map.set(todayStr, [
        makeSlot("08:00", "taken"),
        makeSlot("14:00", "pending"),
        makeSlot("18:00", "pending"),
      ]);

      vi.mocked(mockGetDoseSchedule).mockResolvedValue(map);

      // Range must include today (use UTC midnight timestamp for the date key)
      const todayTs = new Date("2024-06-01T00:00:00Z").getTime();
      const result = await adherenceRate({ start: todayTs, end: todayTs + 86_400_000 });

      // Only the 08:00 slot counts — future pending slots excluded
      expect(result.value.total).toBe(1);
      expect(result.value.taken).toBe(1);
  ...
  it("includes already-actioned future slots (taken/skipped) in denominator for today (D-16)", async () => {
    // System time: 2024-06-01 07:00 UTC = 09:00 local (UTC+2), now = 540 min
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2024-06-01T07:00:00Z"));
    ...
    try {
      ...
      const map = new Map<string, DoseSlot[]>();
      // 08:00 local (480 min) — past (< 540), taken → counted
      // 14:00 local (840 min) — future, taken early → still counted (actioned)
      // 16:00 local (960 min) — future, skipped → counted (actioned)
      // 18:00 local (1080 min) — future, pending → excluded
      map.set(todayStr, [
        makeSlot("08:00", "taken"),
        makeSlot("14:00", "taken"),
        makeSlot("16:00", "skipped"),
        makeSlot("18:00", "pending"),
      ]);
      ...
      // 3 countable: 08:00 taken, 14:00 taken, 16:00 skipped; 18:00 pending excluded
      expect(result.value.total).toBe(3);
      expect(result.value.taken).toBe(2);
      expect(result.value.rate).toBeCloseTo(2 / 3);
    } finally {
      vi.useRealTimers();
    }
  });
