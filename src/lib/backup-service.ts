...
import { db, type IntakeRecord, type WeightRecord, type BloodPressureRecord } from "./db";
...
export async function exportBackup(): Promise<Blob> {
  ...
  logAudit("data_export", `Exported ${intakeRecords.length} intake, ${weightRecords.length} weight, ${bloodPressureRecords.length} BP records`);
...
export async function importBackup(
  ...
  try {
    ...
    if (mode === "merge") {
      const [intakeIds, weightIds, bpIds] = await Promise.all([
    ...
    const totalImported = result.intakeImported + result.weightImported + result.bpImported;
