...
const DOSE_MULTIPLIERS = [0.25, 0.5, 1, 1.5, 2, 3];
    ...
    try {
      ...
      if (selectedPrescriptionId !== "new") {
        ...
        await addPrescriptionMutation.mutateAsync({
          ...
          ...(searchResult?.drugClass && { drugClass: searchResult.drugClass }),
          ...(searchResult?.mechanismOfAction && { mechanismOfAction: searchResult.mechanismOfAction }),
          ...(searchResult?.commonIndications && searchResult.commonIndications.length > 0 && { commonIndications: searchResult.commonIndications }),
          ...(searchResult?.dosageStrengths && searchResult.dosageStrengths.length > 0 && { dosageStrengths: searchResult.dosageStrengths }),
          foodInstruction,
          ...(foodNote && { foodNote }),
          currentStock: parseInt(currentStock) || 0,
          ...(refillDays !== undefined && { refillAlertDays: refillDays }),
        ...
        <div className="grid grid-cols-3 gap-2">
          {DOSE_MULTIPLIERS.map((mult) => {
            ...
            return (
              <button
                key={mult}
                onClick={() => { onDosageAmountChange(mult); onCustomDosageChange(""); }}
                className={cn(
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            if (!isNaN(mgVal) && strengthNum > 0) {
              onCustomDosageChange(String(mgVal / strengthNum));
            } else {
...
function ScheduleStep({
  ...
  const allDaysSelected = (sched: ScheduleEntry) =>
    ALL_DAYS.every(d => sched.daysOfWeek.includes(d));
