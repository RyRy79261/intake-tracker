    ...
    <section aria-label="Daily intake summary">
      <div className="rounded-lg bg-muted/50 border p-4">
        ...
        <div className="grid grid-cols-[auto_repeat(7,1fr)] gap-x-1 gap-y-1">
          ...
          {[
            { key: "water", label: "Water", data: weeklyWater, theme: CARD_THEMES.water, limit: waterLimit, buffer: waterExtendedBuffer, fmt: formatValue },
            { key: "salt", label: "Na", data: weeklySalt, theme: CARD_THEMES.salt, limit: saltLimit, buffer: saltExtendedBuffer, fmt: formatValue },
            { key: "sugar", label: "Sug", data: weeklySugar, theme: CARD_THEMES.sugar, limit: sugarLimit, buffer: sugarExtendedBuffer, fmt: formatValue },
            { key: "caf", label: "Caf", data: weeklyCaffeine, theme: CARD_THEMES.caffeine, limit: 0, buffer: 0, fmt: (v: number) => formatValue(Math.round(v)) },
            { key: "alc", label: "Alc", data: weeklyAlcohol, theme: CARD_THEMES.alcohol, limit: 0, buffer: 0, fmt: (v: number) => v.toFixed(1) },
          ].map((row) => (
